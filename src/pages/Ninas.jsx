...
const Ninas = () => {
  ...
  // Obtener IDs de categorías relacionadas con niñas: 'nina' y 'bebe-nina'
  useEffect(() => {
    const fetchCategoryIds = async () => {
      try {
        const response = await categoryService.getAll();
        const categories = response.data.data || [];
        // Buscar categorías de niñas
        const ninasCategories = categories.filter(cat => 
          cat.slug === 'nina' || cat.slug === 'bebe-nina'
        );
        const ids = ninasCategories.map(cat => cat.id);
        setCategoryIds(ids);
  ...
  // Filtrar productos de niñas usando los IDs de categorías
  const apiFilters = { 
    ...filters 
  };
  
  // Si no hay filtro de categoría específico del usuario, usar las categorías de niñas
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Niñas</h1>
            <p className="text-gray-600 mt-1">Ropa y accesorios para niñas.</p>
          </div>
